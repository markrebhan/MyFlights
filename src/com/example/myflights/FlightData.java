...
public class FlightData {
	...
	public Cursor query(boolean viewAll) {
		...
		//String[] columns = { C_ID, C_ORIGIN, C_DESTINATION, C_DEPART_TIME, C_ARRIVAL_TIME,
				//C_AIRLINE, C_FLIGHT, C_STATUS };
		//Cursor cursor = db.query(true, TABLE, columns, C_IS_DELETED + " = 0",
				//null, null, null, C_DEPART_TIME, null);
		
		String WhereIsDeleted = "WHERE is_deleted = 0";
		// show deleted records if preferences is set to view all
		if (viewAll) WhereIsDeleted = "";
		
		// here is a nasty SELECT statement
		// remember that a the simplecursoradapter is looking for specific names of columns.
		Cursor cursor = db.rawQuery("SELECT f._id, p1.airport AS origin, p1.name AS origin_name, p2.airport AS destination, p2.name AS destination_name, " +
				...
				" ON f.airline = l._id " + WhereIsDeleted + " ORDER BY f.depart_time", null);
	...
	public int queryEntityID(String entity, String table, String column) {
		...
		Cursor cursor = db.query(table, columns, column + "=?", selectionArgs, null, null, null);
	...
	// Check if there is already a duplicate entry based on airline, flight, and depart time
	...
	public boolean queryForDup(int airlineCode, String flight, String departTime) {
		db = dbHelper.getReadableDatabase();
		String [] columns = {C_ID};
		String [] selectionArgs = {Integer.toString(airlineCode), flight, departTime, departTime};
		Log.d(TAG, Integer.toString(airlineCode) + " " + flight + " " + departTime);
		Cursor cursor = db.query(TABLE, columns, C_AIRLINE + "=? AND " + C_FLIGHT + "=? AND " + C_DEPART_TIME + ">? AND " + C_DEPART_TIME + "<? + 86400" , selectionArgs, null, null, null);
	...
	//TODO split up into two update statements to better handle both cases (API enabled or no)
	public int updateDeleted() {
		...
		int rowsAffected = db.update(TABLE, values, "((" + C_ARRIVAL_TIME + "<"
				+ currentDate + ") OR (" + C_DEPART_TIME + "<"
				+ delayedDate + " AND " + C_ARRIVAL_TIME
				+ " IS NULL))" + " AND " + C_IS_DELETED + " = 0", null);
		return rowsAffected;
